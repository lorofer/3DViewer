...
s21::WireframeControllerWidget::WireframeControllerWidget(QWidget *parent)
    : QWidget(parent) {
  AddMoveSliders();
  AddRotateSliders();
  AddScaleSliders();
...
void s21::WireframeControllerWidget::UpdateWireframeInfo(std::string filename,
                                                         int vertices_count,
                                                         int edges_count) {
...
void s21::WireframeControllerWidget::AddChooseFileButton() {
  ...
  connect(choose_file_btn_, SIGNAL(clicked()), this,
          SLOT(OpenChooseFileDialog()));
}
...
void s21::WireframeControllerWidget::AddChooseFileDialog() {
  ...
  connect(choose_file_dialog_, SIGNAL(fileSelected(const QString &)), parent(),
          SLOT(SetWireframeFromFile(const QString &)));
}
...
s21::SlidersGroup::SlidersGroup(QWidget *parent, QString label)
    : QWidget(parent) {
...
void s21::SlidersGroup::AddStretch() { layout_->addStretch(); }

// --------------------

s21::Slider::Slider(QWidget *parent, QString left_label, QString right_label)
    : QWidget(parent) {
...
void s21::Slider::SetRange(int min, int max) { slider_->setRange(min, max); }

void s21::Slider::SetValue(int value) { slider_->setValue(value); }
...
void s21::WifeframeInfo::UpdateWireframeInfo(std::string filename,
                                             int vertices_count,
                                             int edges_count) {
